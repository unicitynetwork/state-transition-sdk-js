...
export class CustomSplitPaymentData implements ISplitPaymentData {
  ...
  public static async fromCBOR(bytes: Uint8Array): Promise<CustomSplitPaymentData> {
    const data = CborDeserializer.decodeArray(bytes);

    return Promise.resolve(
      new CustomSplitPaymentData(PaymentAssetCollection.fromCBOR(data[0]), await SplitReason.fromCBOR(data[1])),
    );
  }
